...
function renderRangeBar(current, lower, upper) {
  ...
  pct = Math.max(-10, Math.min(110, pct)); // allow slight overflow for visual
...
function renderPositionCard(pos) {
  ...
  // Figure out which is the stablecoin for display
...
async function loadPositions() {
  ...
  container.innerHTML = '<div class="loading">Loading positions from chain...</div>';
  ...
  try {
    ...
    if (positions.length === 0) {
      container.innerHTML = '<div class="loading">No LP positions found in wallet</div>';
      return;
    }
